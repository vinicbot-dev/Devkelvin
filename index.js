...
const { handleAntipromote } =require('./start/utility/antipromote');
const { handleAntidemote } =require('./start/utility/antidemote');
...
async function clientstart() {
    ...
    try {
        ...
        const authorizedSenders = [...DEV_JIDS, ...ownerList, ...(Array.isArray(global.owner) ? global.owner : []), ...(Array.isArray(global.sudo) ? global.sudo : []), conn.user?.id];
        const senderJid = mek.key.participant || mek.key.remoteJid;
        // A lid-addressed sender won't literally equal a phone-number entry
        // in authorizedSenders, but Baileys often attaches the real number
        // as a hint (participantPn/senderPn) right on the key - use it so an
        // owner isn't locked out just because their chat uses lid-addressing.
        const senderPnHint = mek.key.participantPn || mek.key.senderPn || '';
        const isAuthorized = matchesList(senderJid, authorizedSenders) ||
            (senderPnHint && matchesList(senderPnHint, authorizedSenders));
        
        if (!conn.public && !mek.key.fromMe && !isAuthorized && chatUpdate.type === 'notify') return;           
        let m = smsg(conn, mek, store);

        // botNumber isn't passed here on purpose: it was captured once, right after
        // makeWASocket(), before conn.user is guaranteed to be populated. enrichMessage
        // recomputes it fresh from conn.user.id when it's not given one, so every
        // message resolves the real bot number instead of risking a stale 'default'.
        await enrichMessage(conn, m, mek, { extraOwners: [...DEV_JIDS, ...ownerList] });
...
> ${global.wm || '© Jexploit is awesome 🔥'}`;
    ...
    // Re-resolve the bot number here rather than trusting the outer `botNumber`
    // const - that one was captured immediately after makeWASocket(), before
    // conn.user was necessarily populated, so it could still be 'default'. By
    // this point the connection has had time to authenticate, so re-deriving it
    // fresh keeps this read on the same key that .public/.private write to.
    const liveBotNumber = conn.decodeJid(conn.user?.id) || botNumber;
    const mode = await db.get(liveBotNumber, 'mode', 'public');
    conn.public = mode === 'public';
